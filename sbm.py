    ...
    for clientes_por_segundo in taxa_entrada:
        ...
        while not teste_passou:
            ...
            if N <= len(tempos_espera):
                observacoes = tempos_espera[:B*M]
                for i in range(len(observacoes)):
                    bloco.append(observacoes[i])
                    if(len(bloco) == M):
                ...
                if RVN < beta:
                    ...
                    print(f"h/media = {h/media_global} M = {M} Clientes = {len(tempos_espera)} taxa entrada = {clientes_por_segundo}")
                ...
                tempos_espera, tempo_chegada_relogio, tempo_final_servico = adicionar_observacoes(tempos_espera, 1000, clientes_por_segundo, taxa_servico, tempo_chegada_relogio, tempo_final_servico)
                M = 5
