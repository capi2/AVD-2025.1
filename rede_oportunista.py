...
def simulacoes(num_simulacoes, nos, R, D, tolerancia_tempo, file):
    ...
    entregas2 = 0
    saltos_total2 = 0
    file.write(f"######### Simulação com nós egoístas sem prioridade #########\n")
    for i in range(num_simulacoes):
        file.write(f"-------------- SIMULAÇÃO {i+1} ------------------\n")
    ...
    file.write("\n---------- RESULTADOS SIMULAÇÃO  NÓS 100% NÃO EGOÍSTAS----------------\n")
    file.write(f"num simulacoes:  {num_simulacoes} | entregas: {entregas1} | taxa entrega: {(entregas1/num_simulacoes)*100}% | saltos: {saltos_total1} | salto medio: {saltos_total1/100}\n")

    file.write("\n---------- RESULTADOS SIMULAÇÃO NÓS EGOÍSTAS COM MENSAGENS SEM PRIORIDADE ----------------\n")
    file.write(f"num simulacoes:  {num_simulacoes} | entregas: {entregas2} | taxa entrega: {(entregas2/num_simulacoes)*100}% | saltos: {saltos_total2} | salto medio: {saltos_total2/100}\n")
